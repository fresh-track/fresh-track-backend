...
const bearerToken = token => {
  return request
    ...
    .then(res => folderContents(res.body.links[0]));
};
...
const folderContents = folder => {
  ...
  return request
    ...
    .then(res => {
      ...
      const found = getUrls(stringRes);
      found.forEach(string => {
        if((string.includes('.wav') || (string.includes('.mp3'))) && string.includes('dl=0')) {
          const fileToAdd = string.split('0%5C')[0];
          dboxlist.push(`${fileToAdd}1`);
        }
      });
